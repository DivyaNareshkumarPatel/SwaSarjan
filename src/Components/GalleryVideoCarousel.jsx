...
const GalleryVideoCarousel = () => {
    ...
    const NextArrow = ({ onClick }) => (
        <div className="arrow next" onClick={() => handleArrowClick("next", onClick)}>
            <FaArrowRight />
        </div>
    );

    const PrevArrow = ({ onClick }) => (
        <div className="arrow prev" onClick={() => handleArrowClick("prev", onClick)}>
            <FaArrowLeft />
        </div>
    );
    ...
    const handleArrowClick = (direction, onClick) => {
        if (direction === "next") {
            setSlideIndex((prevIndex) => (prevIndex === videos.length - 1 ? 0 : prevIndex + 1));
        } else if (direction === "prev") {
            setSlideIndex((prevIndex) => (prevIndex === 0 ? videos.length - 1 : prevIndex - 1));
        }
        onClick();
    };

    const getVideoSize = (index) => {
        const distance = Math.min(Math.abs(index - slideIndex), videos.length - Math.abs(index - slideIndex));
